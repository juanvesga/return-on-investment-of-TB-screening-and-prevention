function results=get_interventions_noTPT(x,sfin,path,prm,ref,sel,agg,gps,location,scen,hivpoints,fityr,endyr)
...
% handles
if (scen==1) % Basic
    fn_mk = @(p,r,intvn_seq)  make_intervention1_noTPT(p, r, i, s, gps,intvn_seq);

elseif (scen==2) % Enhanced
    fn_mk = @(p,r,intvn_seq)  make_intervention2_noTPT(p, r, i, s, gps,intvn_seq);
end
...
txt=sprintf('%s','itvnoTPT','_',num2str(scen));
