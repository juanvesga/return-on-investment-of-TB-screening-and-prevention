function results=get_interventions_slow(x,sfin,path,prm,ref,sel,agg,gps,location,scen,hivpoints,fityr,endyr,varargin)
...
% handles
if (scen==1) % Basic
    fn_mk = @(p,r,intvn_seq)  make_intervention1(p, r, i, s, gps,intvn_seq);

elseif (scen==2) % Enhanced
    fn_mk = @(p,r,intvn_seq)  make_intervention2(p, r, i, s, gps,intvn_seq);
end
...
% profile on
    ...
    getsol = @(Mfinal) ode45(@(t,in)...
        goveqs_scaleup_itv(t, in, M0, Mfinal,[2023 2050], i,s,r,p, sel, agg,hivpoints),...
...
txt=sprintf('%s','itvslow','_',num2str(scen));
