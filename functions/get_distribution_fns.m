...
elseif strcmp(distribution, 'norm')
    ff  = @(x) [normcdf(dat(1),x(1),x(2)), normcdf(dat(2),x(1),x(2)), normcdf(dat(3),x(1),x(2))];
    % Set up initial guess
    ...
    logfn=@(x) sum(-0.5*log(2*pi)-log(sig)-0.5*( (x-mu)/sig ).^2);
...
elseif strcmp(distribution, 'uniform')
    ff  = @(x) [unifcdf(dat(1),x(1),x(2)), unifcdf(dat(2),x(1),x(2)), unifcdf(dat(3),x(1),x(2))];
    % Set up initial guess
    tmp = mn*(1-mn)/var-1; a = tmp*mn; b = tmp*(1-mn); init = [a b];
    % Do the optimisation
    obj = @(x) sum((ff(x)./([2.5 50 97.5]/100) - 1).^2);
...
if nargin == 3 && visualising
    ...
    elseif strcmp(distribution,'norm')
        y = normpdf(x,out(1),out(2));
    elseif strcmp(distribution,'beta')
        y = betapdf(x,out(1),out(2));
        
    elseif strcmp(distribution,'uniform')
        y = unifpdf(x,out(1),out(2));
    end
    plot(x,y); hold on;
