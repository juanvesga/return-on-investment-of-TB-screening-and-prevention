...
function [out, lam_ds, lam_mdr] = governing_equations_itv2(t, in, M, i, s, r, p, sel, agg, growth,hivpoints)
...
if (t<2010)
    ...
    tstPLHIV=(agg.tst_plhiv*(sel.ipt.*M.screen_hiv))*invec;

    tstSLUM= (agg.tpt_slum*(sel.ipt_slum.*M.slumlin.*slum_itv))*invec;
    tstHHC= (agg.tpt_hhc*(sel.ipt_hhc.*M.linHHC))*invec;

    auxil(i.aux.tst_all)= tstPLHIV+tstHHC+tstSLUM;
    auxil(i.aux.tst_plhiv)= tstPLHIV;
    auxil(i.aux.tst_hhc)= tstHHC;
    auxil(i.aux.tst_slum)= tstSLUM;
